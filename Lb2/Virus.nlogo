turtles-own
  [ sick?                  
    remaining-immunity     ;; weeks of immunity left
    sick-time              
    age                    
    age-infection-factor   
  ]
...
to setup-constants
  ...
  set base-infectiousness 65             
  set age-influence 1.5    
  set mutation-timer 0
  set mutation-interval 52
end
...
;; Infection based on turtle's age and general infectiousness level
to infect
  let infection-probability base-infectiousness * (age-infection-factor * (age / lifespan))
  
  ask other turtles-here with [
    not sick? and remaining-immunity <= 0  
  ]
...
to get-older
  set age age + 1
  if age > lifespan [ die ]
  if remaining-immunity > 0 [ set remaining-immunity remaining-immunity - 1 ]  ;; зменшення імунітету з часом
  if sick? [ set sick-time sick-time + 1 ]
end

;; Adjust chance of reproduction based on % infected
to reproduce
  let adjusted-reproduction chance-reproduce * (1 - (%infected / 100))
  if count turtles < carrying-capacity and random-float 100 < adjusted-reproduction
    [ hatch 1
      [ set age 1
        lt 45 fd 1
        get-healthy ] ]
...
to update-mutation
  set mutation-timer mutation-timer + 1
  if mutation-timer >= mutation-interval [
    ; Змінюємо параметри мутації
    set base-infectiousness base-infectiousness * (0.9 + random-float 0.2)  
    set duration duration + random -3 + random 7                          
    set mutation-timer 0                                                  
  ]
end
